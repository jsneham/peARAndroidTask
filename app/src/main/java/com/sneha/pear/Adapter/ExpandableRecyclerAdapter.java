...
public abstract class ExpandableRecyclerAdapter<PVH extends ParentViewHolder, CVH extends ChildViewHolder>
    ...
    public void onParentListItemExpanded(int position) {
        ...
        for(int idx = 0; idx < mItemList.size(); idx++) {
            if (idx != position && idx < mItemList.size() - 1) {
               // collapseParent(idx);
            }
        }
        position = mItemList.indexOf(parent);
        LinearLayoutManager llm = (LinearLayoutManager) mAttachedRecyclerViewPool.get(0).getLayoutManager();
        llm.scrollToPositionWithOffset(position, 300);
    ...
    private void expandParentListItem(ParentWrapper parentWrapper, int parentIndex, boolean expansionTriggeredByListItemClick) {
        if (!parentWrapper.isExpanded()) {
            ...
            if (expansionTriggeredByListItemClick && mExpandCollapseListener != null) {
                ...
                mExpandCollapseListener.onListItemExpanded(parentIndex - expandedCountBeforePosition); //sneha


            }

//            mAttachedRecyclerViewPool.get(0).smoothScrollToPosition(parentIndex+1 + parentWrapper.getParentListItem().getChildItemList().size());
